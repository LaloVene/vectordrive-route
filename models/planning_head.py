...
class PlanningHead(nn.Module):
    ...
    def __init__(self, anchors=None, C_bev=80, C_trans=256, K_anchors=16):
        ...
        if anchors is None:
            # Fallback zero initialization if no K-Means anchors provided
            anchors = torch.zeros(K_anchors, 8, 2)
        ...
        # 3. Anchor projection layer: maps flattened anchor (16) to embedding dimension (C_trans)
        self.anchor_proj = nn.Linear(16, C_trans)
        
        # 4. Multi-head cross-attention layer
        self.cross_attn = nn.MultiheadAttention(embed_dim=C_trans, num_heads=8, batch_first=True, dropout=0.1)
