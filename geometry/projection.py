...
class CylindricalProjection(nn.Module):
    ...
    def __init__(self, H_out=256, W_out=768, H_in=900, W_in=1600, 
        ...
        # u_cyl columns (x direction), v_cyl rows (y direction)
        u_cyl, v_cyl = torch.meshgrid(
    ...
    def forward(self, images, intrinsics, extrinsics):
        ...
        calib_hash = (float(intrinsics.sum().cpu().item()), float(extrinsics.sum().cpu().item()), B, str(device), H_in_actual, W_in_actual)
        if self.cached_calib_hash == calib_hash and self.cached_grid is not None:
            ...
            grid, camera_mask, seam_mask, validity_mask = self._compute_grids(B, intrinsics, extrinsics, device, H_in_actual, W_in_actual)
        ...
        output_images = torch.zeros((B, 3, self.H_out, self.W_out), device=device, dtype=images.dtype)
        for c in range(3):
            ...
            sampled = F.grid_sample(img_c, grid_c, mode='bilinear', padding_mode='zeros', align_corners=False)
    ...
    def _compute_grids(self, B, intrinsics, extrinsics, device, H_in, W_in):
        ...
        large_val = 1e9
        adjusted_dist = torch.where(valid_masks, dist_to_center, torch.full_like(dist_to_center, large_val))
        best_cam_idx = torch.argmin(adjusted_dist, dim=1) # (B, N)
        
        # Construct camera mask representation
        camera_mask_flat = F.one_hot(best_cam_idx, num_classes=3).permute(0, 2, 1).to(device)
        ...
        for c in range(3):
            u_norm = (2.0 * coords_x[:, c]) / (W_in - 1.0) - 1.0
            v_norm = (2.0 * coords_y[:, c]) / (H_in - 1.0) - 1.0
