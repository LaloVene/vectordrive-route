...
class VoxelPooling(nn.Module):
    ...
    def __init__(
        ...
        # Use (H_feat, W_feat) ordering for rows (v) and cols (u)
        v_feat, u_feat = torch.meshgrid(
    ...
    def forward(self, frustum_tensor):
        ...
        output = torch.zeros(
            (B, C, out_size), device=device, dtype=frustum_tensor.dtype
        )

        # Accumulate frustum features into grid using scatter_add for all 4 corners
        for i in range(4):
            idx = (
                getattr(self, f"flat_idx_{i}")
                .unsqueeze(0)
                .unsqueeze(0)
                .expand(B, C, -1)
            )
            weight = (
                getattr(self, f"weight_{i}").unsqueeze(0).unsqueeze(0)
            )  # (1, 1, D * H * W)
