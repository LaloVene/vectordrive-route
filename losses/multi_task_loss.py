...
class MultiTaskLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        bin_indices = torch.clamp(
            torch.round((depth_target - 2.0) / (40.0 / 39.0)).long(), min=0, max=39
        )
        ...
        loss_depth_grid = (
            -0.25
            * ((1.0 - pred_prob) ** 2)
            * torch.log(torch.clamp(pred_prob, min=1e-6))
        )
        loss_depth = torch.sum(loss_depth_grid * depth_mask) / torch.clamp(
            depth_mask.sum(), min=1.0
        )
        ...
        dist_sq = torch.sum(
            (trajectories - future_traj.unsqueeze(1)) ** 2, dim=(-2, -1)
        )  # (B, K)
        ...
        # Convert metric coords to pixel indices on the occupancy grid
        # occupancy grid: X in [-20, 20] maps to columns [0, W_bev-1]
        #                 Y in [0, 40] maps to rows [H_bev-1, 0] (row 0 corresponds to 40m)
        W_bev = occupancy_probs.shape[-1]
        H_bev = occupancy_probs.shape[-2]

        x_cell = (X - (-20.0)) / (40.0) * float(W_bev)  # maps [-20,20] -> [0, W_bev]
        y_cell = (40.0 - Y) / (40.0) * float(H_bev)  # maps [0,40] -> [H_bev, 0]

        # For grid_sample with align_corners=False, normalize as x_norm = 2*(x + 0.5)/W - 1
        u_norm = 2.0 * (x_cell + 0.5) / float(W_bev) - 1.0
        v_norm = 2.0 * (y_cell + 0.5) / float(H_bev) - 1.0

        # Clamp to avoid sampling exactly on border which can produce NaNs in gradients
        u_norm = torch.clamp(u_norm, min=-0.999, max=0.999)
        v_norm = torch.clamp(v_norm, min=-0.999, max=0.999)
        ...
        # sampled shape: (B, 1, K*8, 1) -> reshape to (B, K, 8)
        sampled_occupancy = sampled.view(
            B, 1, trajectories.shape[1], trajectories.shape[2]
        ).squeeze(1)
        ...
        loss_safety = torch.sum(pred_probs * collision_prob) / float(B)

        # 5. Auxiliary Grid Decoders Losses
        drivable_preds = predictions["drivable_preds"]
        drivable_target = targets["drivable_target"]
        loss_drivable = binary_focal_loss_with_logits(
            drivable_preds, drivable_target, alpha=0.5, gamma=2.0
        )
        ...
        y_coords = (
            torch.linspace(40.0, 0.0, 100, device=device).view(100, 1).repeat(1, 100)
        )
        distance_weights = 1.0 / (y_coords + 1.0)
        distance_weights = (
            distance_weights / distance_weights.mean()
