...
def get_args():
    ...
    parser.add_argument(
        "--version", type=str, default="v1.0-mini", help="NuScenes dataset version"
    )
    parser.add_argument(
        "--dataroot",
        type=str,
    ...
    parser.add_argument(
        "--downscale_factor", type=int, default=2, help="Image downscaling factor"
    )
    parser.add_argument(
        "--save_dir",
        type=str,
...
def main():
    ...
    epochs_no_improve = 0
    patience = 3
    history = {"train_losses": [], "val_losses": [], "epochs": []}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 5 == 0 or (batch_idx + 1) == len(train_loader):
                print(
                    f"  Batch {batch_idx+1}/{len(train_loader)} | Total Loss: {loss.item():.4f} | Reg: {losses['loss_reg'].item():.4f} | Safety: {losses['loss_safety'].item():.4f}"
                )
        ...
        print(
            f"  Train Loss: {train_loss_epoch:.4f} (Depth: {train_loss_details['loss_depth']:.4f}, Cls: {train_loss_details['loss_cls']:.4f}, Reg: {train_loss_details['loss_reg']:.4f})"
        )
        print(
            f"  Val Loss:   {val_loss_epoch:.4f} (Depth: {val_loss_details['loss_depth']:.4f}, Cls: {val_loss_details['loss_cls']:.4f}, Reg: {val_loss_details['loss_reg']:.4f})"
        )
    ...
    try:
        ...
        plt.plot(
            history["epochs"], history["train_losses"], label="Train Loss", marker="o"
        )
